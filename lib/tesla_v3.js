...
const mmfaPassCodeParam = 'MFA code';
    ...
    async getNewTokens(params) {
      try {
        const config = {
            ...
            mfaPassCode: params[mmfaPassCodeParam],
...
//      });
      return refreshTokenAsync(oauth.refresh_token);
    }
...
//      return await this.teslaApiGet('/api/1/vehicles/' + id);
      const options = await this.getTJOptions(id);
      return await tjs.vehiclesAsync(options);
    }
