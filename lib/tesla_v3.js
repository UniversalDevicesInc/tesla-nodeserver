  ...
  class TeslaInterface {
    ...
    async getNewTokens(params) {
      try {
        ...
        if (token)
          logger.debug("Login Successful! %s", token);
    ...
    async refreshTokens(oauth) {
      const refresh = await tjs.refreshTokenAsync(oauth.refresh_token);
...
//          new Date((oauth.created_at + oauth.expires_in) * 1000) : null;
      // TODO: test five minute expiration
        const tokenExpiry = oauth && oauth.created_at && oauth.expires_in ?
            new Date((oauth.created_at + 300) * 1000) : null;
      ...
      if ((tokenExpiry && new Date().valueOf() + 60000 > tokenExpiry.valueOf())
        ...
        try {
          const refreshResponse = await this.refreshTokens(oauth);
          if (refreshResponse.statusCode === 401) {
            this.clearTokens();
          }
          oauth = refreshResponse.body;
        } catch (err) {
